...
class offlinedataset(Dataset):
    def __init__(self,
                 ...
                 env_name: str = '',):
        self.eval_env = env
        dataset = self.load_offline_dataset(env_name)
        ...
        super().__init__(np.array(dataset['observations']).astype(np.float32),
                         actions=np.array(dataset['actions']).astype(np.float32),
                         rewards=np.array(dataset['rewards']).astype(np.float32),
                         masks=1.0 - np.array(dataset['terminals']).astype(np.float32),
                         dones_float=np.array(dones_float).astype(np.float32),
                         next_observations=np.array(dataset['next_observations']).astype(
                             np.float32),
                         size=len(dataset['observations'])-1)

    def load_offline_dataset(self,env_name):
        env = self.eval_env
